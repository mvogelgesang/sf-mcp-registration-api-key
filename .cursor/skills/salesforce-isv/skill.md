...
- **Default to User Mode:** All Database operations must explicitly enforce FLS/CRUD.
  - **SOQL:** Use `WITH USER_MODE` in _every_ query.
...
- **Pattern:**
  ...
  2. Assign a Permission Set that _lacks_ access to the target field/object.
  3. Run the code inside `System.runAs(user)`.
  4. **Assert that an Exception is thrown.**
  - _Example:_ "Ensure `System.QueryException` is thrown when a user lacks Read access."
